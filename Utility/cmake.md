...
转载于: https://www.hahack.com/codes/cmake/
...
2. 执行命令 `cmake PATH` 或者 `ccmake PATH` 生成 Makefile[^1]。其中 PATH 是 CMakeLists.txt 所在的目录。
3. 使用 make 命令进行编译。
...
通过上面的定制，生成的 Demo 文件和 MathFunctions 函数库 libMathFunctions.o 文件将会被复制到 `/usr/local/bin` 中，而 MathFunctions.h 和生成的 config.h 文件则会被复制到 `/usr/local/include` 中。我们可以验证一下[^3]
...
[^1]: `ccmake` 和 `cmake` 的区别在于前者提供了一个交互式的界面。
[^2]: 也可以使用 `cmake -i` 命令，该命令会提供一个会话式的交互式配置界面。
[^3]: 顺带一提的是，这里的 `/usr/local/` 是默认安装到的根目录，可以通过修改 `CMAKE_INSTALL_PREFIX` 变量的值来指定这些文件应该拷贝到哪个根目录。
